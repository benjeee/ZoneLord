...
public class ZoneMovement : NetworkBehaviour {
    ...
    void RpcMoveToRandomMapPosition()
    {
        if (isServer)
        {
            ...
            while(Vector2.Distance(xz, new Vector2(transform.position.x, transform.position.y)) < 100)
            {
                xz = Random.insideUnitCircle * (GameManager.instance.matchSettings.mapSize - currScale / 2) / 2;
            }
